...
def count_med_strips_with_available_count(name, per_day, per_strip, available_count, verbose=True):
    ...
    number_of_strips = (total_count - available_count) // per_strip + 1

    currently_available_strips = available_count // per_strip

    if verbose:
        print('Medicine {:15s}: Count = {:7.2f} ({:0.2f} per day) | '
              'Currently Available Strips = {:4.2f} (count={:4.2f}) | '
              'Number of strips to buy = {:3.0f} (count~={:4.0f})'.format(
                name, total_count, per_day,
                currently_available_strips, available_count,
                number_of_strips, total_count - available_count
            ))
